...
# --- НАСТРОЙКИ ---
SELECTION_URL = departments["СОНХ"]["selection_url"]  # URL страницы с выбором группы
...
OUTPUT_FILE = "groups_full СОНХ.py"  # Куда сохранять результат
...
async def extract_groups_full() -> dict:
    """
    Парсит сайт расписания и возвращает структуру для groups_full СОНХ.py.
    ...
    async with async_playwright() as p:
        ...
        for attempt in range(10):
            frames = page.frames
            for f in frames:
                if f.url and f.url.startswith(departments["СОНХ"]["frame_url"]):
                    frame = f
        ...
        for group in groups:
            ...
            url = f"https://coworking.tyuiu.ru/shs/all_t/sh.php?action=group&union=0&sid=28703&gr={group_id}&year=2025&vr=1"
            department = "СОНХ"
...
def save_groups_full_py(groups_full: dict, filename: str = OUTPUT_FILE):
    """
    Сохраняет структуру groups_full СОНХ.py в виде python-файла.
...
async def main():
    """
    Основная точка входа: парсит сайт и сохраняет результат в groups_full СОНХ.py.
