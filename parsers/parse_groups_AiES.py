...
# --- НАСТРОЙКИ ---
SELECTION_URL = departments["АиЭС"]["selection_url"]  # URL страницы с выбором группы
...
OUTPUT_FILE = "groups_full АиЭС.py"  # Куда сохранять результат
...
async def extract_groups_full() -> dict:
    """
    Парсит сайт расписания и возвращает структуру для groups_full АиЭС.py.
    ...
    async with async_playwright() as p:
        ...
        for attempt in range(10):
            frames = page.frames
            for f in frames:
                if f.url and f.url.startswith(departments["АиЭС"]["frame_url"]):
                    frame = f
        ...
        for group in groups:
            ...
            url = f"https://coworking.tyuiu.ru/shs/all_t/sh.php?action=group&union=0&sid=28704&gr={group_id}&year=2025&vr=1"
            department = "АиЭС"
            specialty = group_name.split('-')[0]
            result[group_id] = {
                "name": group_name,
...
def save_groups_full_py(groups_full: dict, filename: str = OUTPUT_FILE):
    """
    Сохраняет структуру groups_full АиЭС.py в виде python-файла.
...
async def main():
    """
    Основная точка входа: парсит сайт и сохраняет результат в groups_full АиЭС.py.
...
async def get_group_schedule_url(group_name):
    selection_url = "https://mnokol.tyuiu.ru/site/index.php?option=com_content&view=article&id=1583&Itemid=299"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for attempt in range(7):
                frame = page.frame(url=re.compile(r"coworking\.tyuiu\.ru/shs/aies_t/aies\.php"))
                if frame:
                    print(f"Найден фрейм расписания: {frame.url}")
                    break
...
async def get_schedule_screenshot(group_name: str) -> Optional[str]:
    """
    Получает скриншот расписания для указанной группы.
    Возвращает путь к сохраненному скриншоту или None в случае ошибки.
    """
    selection_url = "https://mnokol.tyuiu.ru/site/index.php?option=com_content&view=article&id=1583&Itemid=299"
    user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if result != 'ok':
                print(f"Ошибка при выборе группы: {result}")
                return None
            
            await asyncio.sleep(7)
            
            # Делаем скриншот
            screenshot_path = f"schedule_{group_name.replace(' ', '_')}.png"
